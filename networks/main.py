...
def build_network(net_name, ae_net=None):
    ...
    implemented_networks = ('mnist_LeNet', 'mnist_DGM_M2', 'mnist_DGM_M1M2',
                            ...
                            'thyroid_mlp','cancer_mlp','kaggle_mlp',
    ...
    if net_name == 'cancer_mlp':
        net = MLP(x_dim=40, h_dims=[32, 16], rep_dim=10, bias=False)

    if net_name == 'cancer_no_days_mlp':
        net = MLP(x_dim=39, h_dims=[32, 16], rep_dim=10, bias=False)

    if net_name == 'kaggle_mlp':
        net = MLP(x_dim=5, h_dims=[32, 16], rep_dim=4, bias=False)
...
def build_autoencoder(net_name):
    ...
    if net_name == 'cancer_mlp':
        ae_net = MLP_Autoencoder(x_dim=40, h_dims=[32, 16], rep_dim=10, bias=False)
    if net_name == 'kaggle_mlp':
        ae_net = MLP_Autoencoder(x_dim=5, h_dims=[32, 16], rep_dim=4, bias=False)
    return ae_net
