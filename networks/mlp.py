...
class MLP(BaseNet):
    ...
    def __init__(self, x_dim, h_dims=[128, 64], rep_dim=32, bias=False):
        ...
        self.rep_dim = rep_dim # gal: representation dimensionality, i.e. dim of the code layer or last layer.
...
class MLP_Autoencoder(BaseNet):
    ...
    def __init__(self, x_dim, h_dims=[128, 64], rep_dim=32, bias=False):
        ...
        self.rep_dim = rep_dim
        print('encoder dimentions',x_dim, h_dims,'decoder dim are just diversed')
...
class Linear_BN_leakyReLU(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=False, eps=1e-04):
        ...
        self.bn = nn.BatchNorm1d(out_features, eps=eps, affine=bias) # Batch Normalization

    def forward(self, x):
        return F.leaky_relu(self.bn(self.linear(x))) #max(0,x)+ negative_slop*min(0,x)
